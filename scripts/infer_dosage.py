...
def calc_dosage(args, params, filenames, infilename, plotfilename, outfilename):
    ...
    try:
        ...
        if len(for_gaussian_fitting) < 10:
            ...
            xd=np.arange(math.ceil(min(x)), math.ceil(max(x)) + 1)
            ...
            header.append('##INFO=<ID=MEI_left_breakpoint,Number=.,Type=String,Description="Mobile element info for left breakpint">\n')
            header.append('##INFO=<ID=MEI_right_breakpoint,Number=.,Type=String,Description="Mobile element info for right breakpint">\n')
            ...
            with open(outfilename, 'w') as outfile:  # vcf file
                ...
                with open(infilename) as infile:
                    for line in infile:
                        ...
                        meinfo=ls[8].split(',', 1)[1]
                        info='SVTYPE=%s;MEPRED=%s;HOMLEN=%d;%s' % (ls[3], 'PASS', homlen, meinfo)
